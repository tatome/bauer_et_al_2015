...
def baseline(d):
    return config(d)['modalities'][0]['activation']['baseline']
...
header = " & ".join(["$\\nu_s$"] + [c[0] for c in columns]) + "\\\\\n"
col_desc = "r|" + "r" * len(columns)

data = sorted(data, key=baseline)
with open(args.outfile, 'w') as outfile:
    outfile.write("""{\\small%\n""")
    outfile.write("\\begin{tabular}{" + col_desc + "}\n")
    outfile.write(header)
    for d in data:
        line = [baseline(d)] + [c[1](d) for c in columns]
        formats = ["%.1f"]    + [c[2]    for c in columns]

        if baseline(d) == baseline(base_data):
            template = "$\\mathbf{%s}$"
        else:
            template = "$%s$"
